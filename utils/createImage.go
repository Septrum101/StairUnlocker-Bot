...
func GeneratePNG(streamDataList []model.StreamData, streamMediaNames []string) (*bytes.Buffer, error) {
	// merge node unlock data
	nodeMap := make(map[string][]model.StreamData)
	for i := range streamDataList {
		nodeMap[streamDataList[i].ProxyName] = append(nodeMap[streamDataList[i].ProxyName], streamDataList[i])
	}
	var sortedNode [][]model.StreamData
